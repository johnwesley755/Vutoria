    ...
    <section className='max_padd_container pt-28'>
      <table className='w-full mx-auto'>
        ...
        <tbody>
            {all_products.map((e) => {
                if(cartItems[e.id] > 0){
                    return <tr key={e.id} className='border-b border-slate-900/20 text-gray-30 p-6 medium-16 text-center'>
                        <td className='flexCenter'>
                            <img src={e.image} alt='prdctImg' height={43} width={43} className='rounded-lg ring-slate-900/5 my-1'/>
                        </td>
